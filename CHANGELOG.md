...
### Added
- `transform/` — dbt-duckdb project scaffold (`dbt_project.yml`, `profiles.yml`, `packages.yml`, `.gitignore`) laying the groundwork for a dbt transformation layer on top of the existing DuckDB warehouse; no models yet
- `dbt-duckdb>=1.7.0,<2.0.0` dependency added to `requirements.txt` and `pyproject.toml`
