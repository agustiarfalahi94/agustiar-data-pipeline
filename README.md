...
- **📍 Locate Me** — centres the map on your current GPS location with a red marker, and moves you
...
- **📍 Arrivals near you** — with your location set, see nearby stops, starting at 800 m
  ...
  inbound first, except that the stop you have selected is always in the list and always first, so
  the map and this list never describe different stops — the map draws a ring for every stop found
  (13 in a typical 800 m neighbourhood here) while the list holds a ranked few, so without that a
  tapped ring often highlighted nothing in the list at all. If stops with buses coming were left
  out, the panel says how many rather than dropping them silently. Each nearby stop is also drawn on the map as a gold ring, faintly
  ...
  (stop rings are gold, buses blue, your own position red). The highlight appears straight away from
  either route. It used to trail a ring tap by one render, which was documented here as a harmless
  cosmetic lag; it was not harmless, because it also meant every *second* tap on the map was thrown
  away (see *A tap that moves the map's selection reruns immediately* under Design Decisions). The panel opens between
  the map and this list, so clicking a name means scrolling *up* to see what you just opened — the
  app does not auto-scroll to it (a JS workaround was rejected in 2.7.0 for fighting the
  auto-refresh rerun). Tap a bus
...
| Decision | Reason |
|---|---|
| **Page summaries are computed in SQL, not in pandas** | 📊 Data Table and 📈 Analytics each called `get_historical_data()` — `SELECT *` over the retention window — on every visit: 602,950 rows and 6.5 seconds, measured, and growing with the table because it is append-only. Data Table displayed one screenful of that; Analytics only ever wanted counts and averages. Both now ask the database for what they show: **6577 ms → 141 ms** and **7034 ms → 314 ms**, measured in a browser with no fetch running. The answers are identical, and that was verified against the old pandas result on the real database rather than assumed — which required two details. The queries convert speed to km/h, round it and cap it at 120 *before* aggregating, because the pages grouped and filtered on the converted value; and they use `round_even`, because pandas rounds halves to even where DuckDB's `round` does not. The speed summary returns sums and counts rather than averages, so Analytics can regroup it per vehicle and per vehicle-and-region without averaging averages |
| **Data Table loads a page, and says so** | The newest 1,000 rows by default, with 10,000 and "all rows" offered. A caption names how many of the total are on screen and the CSV exports exactly those rows — a silently shrunken export would be worse than a slow one. `avg_speed` is still measured over each vehicle's whole history in the selected regions and joined onto the page, so limiting the view cannot quietly redefine the column |
| **Parallel fetch with ThreadPoolExecutor** | Cuts refresh time from ~15s to ~2-3s in good conditions. The pool is sized to the endpoint list rather than fixed at ten, so all fifteen endpoints go in one wave instead of ten then five, and every request shares one pooled HTTPS connection instead of repeating the TLS handshake fifteen times against the same host |
| **The auto-refresh fetch runs off the render path** | `api.data.gov.my` takes 3-6 seconds to answer a single endpoint — even one whose entire reply is 143 bytes — and a whole refresh has been measured at 21-29 seconds when the API is struggling. Reading the result back out of DuckDB takes about 30 ms. So the page was never slow; it stood still in front of a network call, and every 20-second tick froze it for the length of the slowest agency's reply, for data that is *already* 26-124 seconds old on arrival. The fetch now runs on a daemon thread (`utils/background_fetch.py`) and the page renders from whatever the database already holds: measured at ~147 ms per interaction with a fetch in flight. New rows appear on the following tick. The thread touches no Streamlit state — it has no ScriptRunContext, and session state belongs to one session while the thread is shared by all of them — so the page notices the fetch has ended and refreshes its own held frame. Only one fetch runs at a time, or a slow feed would stack a thread per tick. **The manual Refresh Data button still blocks on purpose**: the user pressed it and is waiting for an answer |
| **A background refresh still says it is happening** | A refresh nobody can see is a page you cannot tell from a stuck one. While a fetch runs, the sync line reads `Data updated: … · 🛰️ updating…`, so the signal the blocking spinner used to give survives without the page being held still to give it |
...
| **One `stop_id`, one stop** | `get_stops_near` de-duplicates by `stop_id` before it truncates to the panel's limit. A feed is free to repeat a row, and the id is what every caller treats as the stop's identity: the panel keys one button per stop on it, and Streamlit raises on a repeated widget key — into the render, so one malformed row would cost the whole Live Map rather than one duplicated line. De-duplicating before the truncation rather than after it means a duplicate cannot spend one of the five places a real stop needed; the nearest occurrence is the one kept |
...
| **Locate Me follows the location only at a dead end** | Switching region on *every* locate would overrule a working choice — the user may have selected that region deliberately — and would run `find_regions_with_stops_near`, which reads every other agency's timetable and on a cold deploy downloads them, on an interaction that happens constantly. So the switch fires only when the selected region has no stops within 1500 m: the same dead end the cross-region hint was built for. Permission is a one-shot session flag set by the Locate Me handler and consumed by the stop scan, so a region chosen *after* locating is never overridden. The new region is parked in session state rather than assigned directly, because Streamlit refuses to let a widget's own key be written after the widget exists in that run and the locate handler runs after the dropdown; the rerun it triggers applies it before the dropdown is built. The change is announced next to the dropdown that changed — a region silently swapping under the reader would be worse than the dead end it fixes |
...
| **A stop's name selects it, through the same session key a ring tap sets** | The name in "Arrivals near you" used to be a Google Maps link and nothing else — no way to select a stop without finding its ring on the map first. It is now a button that writes `st.session_state['selected_stop_id']`, the identical key a ring tap writes, so the last-tap-wins rule and the one-shot `cleared_stop_id` suppression both still govern a single selection rather than two. It also clears `selected_vehicle_id`, matching what a ring tap already does — otherwise a bus left selected stays sticky, and the next render shows the bus panel and the freshly-opened stop panel together, exactly what last-tap-wins forbids. It is drawn as a selected control in the list while it is the selection, because the ring and the panel are both above the list and on a phone that means above the fold — without it, clicking a name and staying put reads as nothing having happened. The Google Maps link is kept rather than lost, on the line directly under the name: `st.button` and `st.markdown` are block elements, so it stacks. The design spec's diagram shows it *beside* the name; putting it there means `st.columns([3, 1])`, and a stop name like "KL1743 GREEN AVENUE CONDOMINIUM" in a 75%-width phone column with "Google Maps · ~120 m · ~2 min walk" crammed into the remaining 25% wraps into more lines than the stack it replaced. The diagram is historical |
| **A tap that moves the map's selection reruns immediately** | The stops layer is built from `_selected_stop_id` *before* `st.pydeck_chart()` hands back this render's tap, so the drawn map is always one selection behind. That was documented here as a harmless cosmetic lag on the ring. It was not harmless: the highlight is part of the deck spec, and Streamlit folds the deck spec into the chart's identity, so a tap that moved the selection left the chart on screen carrying an id the *next* render would not rebuild — and the next tap was looked up under an id with no widget state behind it and read as "nothing tapped". Every second tap on the map did nothing, which is exactly how it was reported: tap a stop and the panel is right, tap the next and the panel still names the previous one, tap a third and it catches up. `show()` now reruns as soon as the adopted selection differs from the one the layer was drawn with, so the chart the user taps is always the chart the next render builds. Guarded on an actual change: re-tapping the stop already selected leaves the spec, and so the id, untouched, and Streamlit may redeliver the same payload — an unguarded rerun would spin on it. The guard is self-quieting, because after the rerun the drawn selection matches and a redelivered payload changes nothing. The rerun is cheap because the live frame is held (see above) and auto-refresh no longer fetches on every rerun |
...
| **The selected stop is pinned to the top of "Arrivals near you"** | The map draws a ring for every stop the scan found; the list is a ranked top few. So tapping a ring usually selected a stop the list did not contain, and the list's only feedback — the highlighted name — was missing from the one place the user was looking. The panel above did update, but on a phone it is above the fold, so the tap read as having done nothing. The selected stop is now always listed and listed first, from the same `selected_stop_id` the ring is styled from — no new state, and the two views cannot disagree. Pinning replaces the last ranked entry rather than lengthening the list, and the "more stops have buses coming" count is measured against what was actually listed rather than against the display cap, so it stays true when a pin displaces a ranked stop |
...
- [x] Selected stop's ring is drawn magenta and thicker on the map, instantly from either a name
      click or a ring tap — the one-render lag this used to have was also eating every second tap
