...
| **A selected ring's highlight can lag a ring tap by one render, and that is left alone** | `_selected_stop_id` is read to build the stops layer before `st.pydeck_chart()` has delivered *this* render's click, so a freshly-tapped ring draws with its previous styling for one render — the tap and the panel it opens are still instant, only the ring's own colour/width catch up on the next render (≤20s via auto-refresh). A name click doesn't have this problem, because its own `st.rerun()` rebuilds the layer with the new selection already in session state. Forcing the ring tap to match with a bare `st.rerun()` after adopting the tap was not added: Streamlit hashes the deck spec into the element id even with an explicit key, so adopting a genuinely new stop changes that stop's `line_color`/`line_width` row and lands the rerun on a new id with no stale payload to redeliver — no loop in the common case. The narrower risk is re-tapping the ring of the stop already selected, where the spec (and so the id) does not change and the same payload could come back; a one-line guard skipping the rerun when the adopted stop already matches the current selection would close that case, and without adding state — it reads the selection that is already there. (An earlier version of this note claimed a `deck_generation` bump would risk resetting the user's pan/zoom. That is overstated: in the bundled deck.gl, `setProps` re-seeds the camera only when `initialViewState` itself changes, and a key bump does not change it — the element id already changes on every 20-second data refresh while the viewport stays put. The guard is preferable for being smaller, not for averting that.) Left undone because the highlight already self-heals within one auto-refresh and never affects which panel opens — not because a safe fix doesn't exist |
...
- [x] Selected stop's ring is drawn brighter and thicker on the map — instantly from a name click;
      a ring tap's own highlight can lag one render (≤20s) behind the panel it opens, by design (see
      Design Decisions)
