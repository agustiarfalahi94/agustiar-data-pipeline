...
| **A selected ring's highlight can lag a ring tap by one render, and that is left alone** | `_selected_stop_id` is read to build the stops layer before `st.pydeck_chart()` has delivered *this* render's click, so a freshly-tapped ring draws with its previous styling for one render — the tap and the panel it opens are still instant, only the ring's own colour/width catch up on the next render (≤20s via auto-refresh). A name click doesn't have this problem, because its own `st.rerun()` rebuilds the layer with the new selection already in session state. Forcing the ring tap to match by calling `st.rerun()` the same way was rejected: `on_select="rerun"` keeps redelivering an unchanged click payload until the widget's key (`deck_generation`) changes, so an unconditional rerun here would re-enter its own branch on the very next execution and loop forever; forcing it the way the "Clear" buttons do (bumping `deck_generation`) would remount the pydeck widget on every single tap, risking the user's in-progress pan/zoom for a highlight that already self-heals |
